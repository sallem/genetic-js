var Genetic = (function() {
    ...
    this.start = async function() {
      ...
      for (i = 0; i < this.configuration.iterations; ++i) {
        ...
        if (this.configuration.workerPath && this.configuration.workersCount > 1) {
          ...
          const promises = entitiesChunks.map(async function(entities) {
            return new Promise(function(resolve) {
              const myWorker = self.startWorker(
                self.configuration.workerPath,
                self.configuration.execArgv,
          ...
          // score and sort
          pop = this.entities.map(function(entity) {
  ...
  Genetic.prototype.startWorker = function(path, execArgv, data, cb) {
    const { Worker } = require('worker_threads');
    const workerChild = new Worker(path, {
      execArgv,
      workerData: data,
    });
    workerChild.once('message', message => {
      cb(null, message);
    });
  };
