var Genetic = (function () {
  ...
  function Genetic() {
    ...
    this.start = async function () {
      ...
      async function mutateOrNot(entity) {
        // applies mutation based on mutation probability
        return Math.random() <= self.configuration.mutation && self.mutate ? await self.mutate(Clone(entity)) : entity;
      }
      ...
      for (i = 0; i < this.configuration.iterations; ++i) {
        ...
        while (newPop.length < self.configuration.size) {
          if (
            ...
            var children = await Promise.all((await this.crossover(Clone(parents[0]), Clone(parents[1]))).map(mutateOrNot))
            newPop.push(children[0], children[1]);
          } else {
            newPop.push(await mutateOrNot(self.select1(pop)));
          }
        }
  ...
  Genetic.prototype.startWorker = function (path, workerOptions, data, cb) {
    ...
    // console.log("Start worker :", Object.keys(data.userData.computedValues).length)
